...
import cv2
from core.utils_ori import to_tensors
...
def compare_ssim(img1, img2, data_range=255, multichannel=True, win_size=65):
    K1 = 0.01
    K2 = 0.03
    sigma = 1.5
    ...
    if multichannel:
        ...
        img1_yuv = rgb2yuv(img1)
        img2_yuv = rgb2yuv(img2)

        # Calculate SSIM for each channel (Y, U, V)
        ssim_channels = []
        for i in range(3):
            channel_ssim = calculate_ssim(img1_yuv[:, :, i], img2_yuv[:, :, i], data_range, win_size, K1, K2, sigma)
            ssim_channels.append(channel_ssim)

        # Average the SSIM values from all channels
        ssim = np.mean(ssim_channels)
    else:
        # Calculate SSIM for grayscale images
        ssim = calculate_ssim(img1, img2, data_range, win_size, K1, K2, sigma)

    return ssim

def calculate_ssim(img1, img2, data_range, win_size, K1, K2, sigma):
    C1 = (K1 * data_range) ** 2
    C2 = (K2 * data_range) ** 2
...
def gaussian_filter(img, sigma,win_size=65):
    return cv2.GaussianBlur(img, (win_size, win_size), sigma)
###########################
# I3D models
###########################
