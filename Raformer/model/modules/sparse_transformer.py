...
from .RAA_SFA_model import RAA_SFA
...
class TemporalSparseTransformerBlock(nn.Module):
    def __init__(self, dim, n_head, window_size, pool_size, depths, t2t_params=None):
        ...
        for i in range(depths):
            ...
            window_attention_blocks.append(
                RAA_SFA(window_size=(5,9),input_dim=512)
            )
            
        self.transformer = nn.Sequential(*blocks)
        self.windowatten = nn.Sequential(*window_attention_blocks)
        self.depths = depths
    ...
    def forward(self, x, fold_x_size, l_mask=None, t_dilation=2):
        ...
        for i in range(0, self.depths):
            x = self.transformer[i](x, fold_x_size, l_mask, T_ind[i]) #1,13,20,36,512
            x = self.windowatten[i](x)
